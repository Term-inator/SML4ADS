...
@Time : 2022/2/11-14:38
@Author : zch
@Description :
"""
import random

import src.parser.structs
...
def waypoint_next_try():
    ...
    while True:
        ...
        if ego_wp.is_junction and junction_flag:
            junction_flag = False
            temp_wp, _ = action.compute_next_wpt_vel(ego, VehicleState.TURNLEFT, True, 20, prev_road_id, prev_wp)
        ...
        if ego_wp.transform.location.distance(next_wp.transform.location) < 1:
            prev_wp = next_wp
            next_wp, _ = action.compute_next_wpt_vel(ego, VehicleState.KEEP, False, prev_wp=prev_wp,
                                                     prev_road_id=prev_road_id)
        if next_wp:
            control = controller.run_step(30, next_wp, ego_wp.is_junction)
...
def get_possible_wps_as_incoming(parser: MapParser, junction: src.parser.structs.Junction, in_id: int):
